...
sidebar_position: 2
title: ChartUI
description: Props, theme tokens, hooks, and utilities for @exeria/charts-ui.
---
...
**ChartUI** is the React wrapper from `@exeria/charts-ui`. It renders the top toolbar, left drawing menu, and settings dialogs around a `ChartInstance`.
...
```tsx
import type { ChartUITheme } from "@exeria/charts-ui";
```
...
```tsx
import { useChartEnvironment } from "@exeria/charts-ui";
...
Returns `ChartEnvironmentSnapshot` from `@exeria/charts` — same shape as `chart.getChartEnvironment()`.
