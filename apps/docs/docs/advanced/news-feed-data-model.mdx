...
Types are exported from `@exeria/charts` as `NewsFeedRecord`, `NewsFeedBundle`, and `NewsFeedAdapter`.
...
```ts
import { resolveNewsBarIndex, setInstrumentNewsFeed } from "@exeria/charts";
