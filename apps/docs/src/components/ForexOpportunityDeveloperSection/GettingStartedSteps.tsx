...
const STEPS = [
  {
    ...
    body: (
      <>
        Open the{" "}
        <Link to="/starters/forex-platforms/app">live FX Opportunity Radar</Link> in another tab.
        Opportunity feed, chart scenes, and news dots — no install yet.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <code>http://localhost:5173</code>) — open it. The chart loads EUR/USD candles from{" "}
        <code>public/data/eur-usd-m15.json</code>.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Open <code>src/App.tsx</code> in Cursor, VS Code, or any editor. Change the pair, add a
        strategy script, or tweak colors — save the file and the page updates automatically. The{" "}
        <code>snippets/</code> folder has copy-paste examples for news, opportunities, and backend
        wiring.
      </>
    ),
  },
...
export default function GettingStartedSteps() {
  return (
    <section className={styles.stepsSection} aria-labelledby="forex-opportunity-steps">
      <h3 id="forex-opportunity-steps">Get started in 5 steps</h3>
