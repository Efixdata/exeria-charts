...
const ASSET_GROUPS: AssetGroup[] = [
  {
    ...
    symbols: [
      ...
      { id: "MSFT", name: "Microsoft Corp.", buttonLabel: "MSFT", priceDecimals: 2 },
  ...
  },
  {
    ...
    symbols: [
      ...
      { id: "GBP/USD", name: "British Pound / US Dollar", buttonLabel: "GBP/USD", priceDecimals: 5 },
    ],
  },
  {
    id: "crypto",
...
export default function FinnhubConnectorExample() {
  ...
  const adapterRef = useRef<ProxyFinnhubAdapter | null>(null);
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      ...
      try {
        ...
        adapterRef.current = new ProxyFinnhubAdapter();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setUsingFallback(adapterRef.current?.historicalFallback ?? false);
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {usingFallback && (
        <div
          className={`${styles.dataNotice} ${styles.dataNoticeSynthetic}`}
          role="status"
        >
          <strong>Demo candles:</strong> Your Finnhub plan does not include historical
          OHLCV for this symbol. The chart shows synthesized bars derived from the latest
          quote — not real market history. Subscribe to a{" "}
          <a
            href="https://finnhub.io/pricing-stock-api-market-data"
            target="_blank"
            rel="noreferrer"
          >
            Finnhub market data plan
          </a>{" "}
          for production candle data.
        </div>
      )}
      ...
      {priceData && (
        <div className={styles.priceDisplay}>
          ...
          <div className={styles.info}>
            <p>
              <strong>✓ Finnhub proxy:</strong> Stocks, forex, and crypto via `/api/finnhub`
            </p>
            <p>
              <strong>✓ API key safe:</strong> Server-side only — not bundled in the browser
            </p>
            <p>
              <strong>✓ Free tier:</strong> Intraday history is limited (~1 month); daily bars
              go further back.
            </p>
            <p>
