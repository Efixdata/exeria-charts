...
const SPOT_SYMBOLS = [
  ...
  { id: "ETH_USDT", name: "Ethereum / USDT" },
  { id: "SOL_USDT", name: "Solana / USDT" },
  { id: "GT_USDT", name: "GateToken / USDT" },
];
...
function symbolButtonLabel(symbolId: string): string {
  return symbolId.split("_")[0]?.slice(0, 3) ?? symbolId;
}

export default function GateConnectorExample() {
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      ...
      try {
        ...
        adapterRef.current = new ProxyGateAdapter();
      ...
      {priceData && (
        <div className={styles.priceDisplay}>
          ...
          <div className={styles.info}>
            <p>
              <strong>✓ No API key:</strong> Public Gate.io spot market data
            </p>
            <p>
              <strong>✓ Gate.io proxy:</strong> History and live prices via <code>/api/gate</code>{" "}
              (avoids browser CORS on REST)
            </p>
            <p>
              <strong>✓ Native pairs:</strong> Uses Gate symbols like <code>BTC_USDT</code>
            </p>
          </div>
