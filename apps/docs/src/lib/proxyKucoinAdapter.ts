...
export type ProxyKucoinAdapterConfig = {
  apiBase?: string;
...
async function readApiError(response: Response): Promise<string> {
  ...
  if (isHtmlResponse(text)) {
    return "KuCoin API proxy is unavailable. Restart docs with `npm run dev` (not `docusaurus serve`) so /api/kucoin routes are registered.";
  }
...
async function readJsonPayload<T>(response: Response): Promise<T> {
  ...
  if (isHtmlResponse(text)) {
    throw new Error(
      "KuCoin API proxy returned HTML instead of JSON. Restart docs with `npm run dev` so /api/kucoin routes are registered.",
    );
  }
...
export class ProxyKucoinAdapter implements DataAdapter {
  ...
  constructor(config: ProxyKucoinAdapterConfig = {}) {
    this.apiBase = config.apiBase ?? "/api/kucoin";
