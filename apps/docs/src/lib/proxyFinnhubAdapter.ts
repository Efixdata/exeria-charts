...
export type ProxyFinnhubAdapterConfig = {
  apiBase?: string;
...
export class ProxyFinnhubAdapter implements DataAdapter {
  ...
  /** True when the last OHLCV response used synthesized demo candles (free-tier fallback). */
  historicalFallback = false;

  constructor(config: ProxyFinnhubAdapterConfig = {}) {
    this.apiBase = config.apiBase ?? "/api/finnhub";
  ...
  async getHistoricalData(
    ...
    this.historicalFallback = Boolean(payload.fallback);
