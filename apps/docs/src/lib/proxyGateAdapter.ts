...
export type ProxyGateAdapterConfig = {
  apiBase?: string;
...
async function readApiError(response: Response): Promise<string> {
  ...
  if (isHtmlResponse(text)) {
    return "Gate.io API proxy is unavailable. Restart docs with `npm run dev` (not `docusaurus serve`) so /api/gate routes are registered.";
  }
...
async function readJsonPayload<T>(response: Response): Promise<T> {
  ...
  if (isHtmlResponse(text)) {
    throw new Error(
      "Gate.io API proxy returned HTML instead of JSON. Restart docs with `npm run dev` so /api/gate routes are registered.",
    );
  }
...
export class ProxyGateAdapter implements DataAdapter {
  ...
  constructor(config: ProxyGateAdapterConfig = {}) {
    this.apiBase = config.apiBase ?? "/api/gate";
