...
export default class Chart {
  ...
  init() {
    ...
    this.fusion = new FUSION.builder()
      .setModel(this.model)
      .build()

    this.interactor = new InteractionsController(this.container, this.canvas, this.overlay, this.model, this.renderer, this.topLayer, this.config, this.fusion, this);

    this.fit();
    this.renderer.render(this.ctx, this.model, theme, this.fusion, false, this.objectOnlyOnOverlay);
  ...
  render(objectOnlyOnOverlay) {
		...
		if (!this.isChartEmpty()) {
			this.renderer.render(this.ctx, this.model, theme, this.fusion, false, objectOnlyOnOverlay);
		} else
			this.renderEmpty();
  ...
  fit() {
    if(this.fusion.getMainSeries() && this.fusion.getMainSeries().data && this.fusion.getMainSeries().data.length >0){
			var self = this;
			var r = self.renderer;
	  ...
	  this.overlay.height = this.canvasHeight;
			...
			this.model['_timeAxisWidth']	= this.model._width - this.model.valueAxisWidth;
  ...
  fitPanel(panel, index, offset) {
    if(!this.valueConverter || this.valueConverter.mode !== panel.valueAxisMode)
		...
		var extremesOffset = 0; var extremesMargin = 0;
		...
		if (this.model.autoScale == true && panel.objects.length>0) {
			...
			for (var i=0; i<panel.objects.length; i++) {
				...
				if(!this.fusion.getMainSeries() || !this.fusion.getMainSeries().data) {
					log.trace("Update extreme on object ommited: main series not loaded yet!",panel.objects[i]);
					continue;
				}
				var ext = {min: Number.MAX_VALUE, max: -Number.MAX_VALUE};
				this.renderer.objects[panel.objects[i].type].updateExtremes(panel.objects[i], ext, this.model, this.fusion.getSeriesManager(), panel,this.renderer);

				var fV = LIB.getReferenceValue(panel.objects[i], this.model, this.fusion.getSeriesManager());
			...
			if(Math.abs(extremes.max) == Number.MAX_VALUE && Math.abs(extremes.min) == Number.MAX_VALUE  ){
				//wtf? no extremes?
				extremes.max = 1;
				extremes.min = -1;
			}
			...
			if(panel.centerZero == true){
				...
				if(Math.abs(extremes.min-extremesMargin) > range) range = Math.abs(extremes.min-extremesMargin);
				panel.vMax = range;
				panel.vMin = -range;
			}else{
  ...
  chartStructureChanged(mode){
	...
	renderOverlay() {
		...
		if (!this.isChartEmpty()) {
			...
			if (this.interactor.currentMode.renderOverlay) this.interactor.currentMode.renderOverlay(octx);

			this.renderer.postRenderOverlay(octx, this.model, this.fusion.getSeriesManager());
		}
	}
