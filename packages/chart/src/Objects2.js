...
function TimeRangeObject() {
	...
	this.render	= function (o, ctx, renderer, model, panel, seriesManager) {
		...
		const textHeight = measuredText.fontBoundingBoxAscent || 11;
		ctx.fillStyle = o.textColor ? o.textColor : WEBRCP.utils.colorManager.getColor('defaultToolTextColor');
		ctx.fillText(o.text, (pts[0].x + pts[1].x)/2 - measuredText.width/2, panel._offset+panel._height - (22 - textHeight)/2);
	...
	this.getPoints	= function (o, renderer, panel, model, seriesManager) {
		var pts = TimeRangeObject.prototype.getPoints.call(this , o, renderer, panel, model, seriesManager);
		pts[0].y = panel._offset + panel._height / 2 - this.anchorPointSize / 2;
		pts[1].y = panel._offset + panel._height / 2 - this.anchorPointSize / 2;
		return pts;
	}
}

function FutureTimeRangeObject() {
	...
	this.getPoints	= function (o, renderer, panel, model, seriesManager) {
		...
		const secondStamp = lastStamp + o.period;
		var pts = [
			{
				x: renderer.getIndexPoint(lastIndex, model) + model.periodWidth / 2,
				y: panel._offset + panel._height / 2 - this.anchorPointSize / 2,
				index: lastIndex,
				stamp: lastStamp
			},
			{
				x: renderer.getStampPoint(secondStamp, model, seriesManager) + model.periodWidth / 2,
				y: panel._offset + panel._height / 2 - this.anchorPointSize / 2,
				index: renderer.getStampIndex(secondStamp, model, seriesManager),
				stamp: secondStamp
			}
		];
...
export { Shape, TrendLineObject,  FibonLinesObject, ParallelChannelObject, ArrowObject, HorizontalLineObject, VerticalLineObject, DiNapoliLevels, DiNapoliAbcObject, MultiLineObject, AbcdObject, EllipseObject, HorizontalRangeObject, VerticalRangeObject, TimeRangeObject, FutureTimeRangeObject, CycleObject, TextObject, BoxObject, TriangleObject, PriceTagObject }
