...
function TimeBetObject(this: ShapeRuntime) {
  ...
  this.getColors = function (o: LegacyShapeObject, isWinning: boolean) {
    ...
    const winningColor = o.winningColor
      ? o.winningColor
      : WEBRCP.utils.colorManager.getColor("chartGreen");
    const losingColor = o.losingColor
      ? o.losingColor
  ...
  this.isWinning = function (o: LegacyShapeObject, model: any, seriesManager: any) {
    ...
    if (o.status === "ACTIVE") {
      const lastPrice =
        seriesManager[model.mainSeries].data[seriesManager[model.mainSeries].data.length - 1]["c"];
  ...
  this.render = function (
    ...
    const boxWidth = Math.ceil(
      measuredText.width + boxPadding.left + boxPadding.right + directionBoxWidth
    );
    ...
    ctx.fillText(text, boxBeginningX + directionBoxWidth + boxPadding.left, y0);
    ctx.fillStyle = toolColor;
    ctx.lineWidth = 1;
    ctx.shadowBlur = 2;
