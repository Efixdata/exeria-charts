...
function DiNapoliAbcObject(this: ShapeRuntime) {
  ...
  this.getPoints = function (o, renderer, panel, model, seriesManager) {
    var pts = DiNapoliAbcObject.prototype.getPoints.call(
      this,
      o,
      renderer,
      panel,
  ...
  this.drawLevelPoints = function (pts, o, ctx, renderer, model, panel, seriesManager) {
    ...
    for (var i = 0; i < o.values.length; i++) {
      ...
      if (enabled == true) {
        ...
        if (!expanded) ctx.lineTo(levelX + 130, levelY);
        else {
          ctx.fillStyle = this.getValueColor(level);
          ctx.setLineDash([3, 3]);
      ...
      for (var i = 0; i < pts.length; i++) {
        if (
          ...
          var nlp1 = getLinePointNearestMouse(
            { x0: pts[i].x, y0: pts[i].y, x1: pts[i].x + 100, y1: pts[i].y },
            x,
            y
          );
          ...
          if (
            ...
            between(pts[i].x, x, pts[i].x + 100, self.hitTolerance) &&
            between(pts[i].y, y, pts[i].y, self.hitTolerance)
          ) {
            hitResult = true;
            o._hit = true;
    ...
    if (!this.wasDrag) {
      if (
        interactor.isOver(
          ...
          e._offset.offsetY,
          pts[2].x,
          pts[2].y + this.anchorPointDistanceToArrow,
          self.hitTolerance
        )
      ) {
