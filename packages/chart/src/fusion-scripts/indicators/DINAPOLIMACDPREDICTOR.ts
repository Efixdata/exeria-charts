...
export default function createDINAPOLIMACDPREDICTORIndicatorScript(FUSION: CoreFusionStatic) {
  return {
    ...
    plotters: [
      {
        type: "SeriesObject",
        dataLink: "MACDPredictor",
        renderAs: "Line",
        dataField: "MACDPredictorValue",
        color: "#f44336",
        width: 1.5,
        dash: [],
        priceTag: true,
    ...
    controller: function (
      ...
      var MACDPredictorController: FusionScriptControllerConstructor = function (
        ...
        this.calculate = function (this: any, index: any) {
          ...
          if (this.EMA1.getValue(index - 1) !== null) {
            this.EMA1.setValue(
              index,
              this.EMA1.getValue(index - 1) +
                this.SMOOTHINGFACTOR1 * (this.CLOSE.getValue(index) - this.EMA1.getValue(index - 1))
            );
          } else {
          ...
          if (this.EMA2.getValue(index - 1) !== null) {
            this.EMA2.setValue(
              index,
              this.EMA2.getValue(index - 1) +
                this.SMOOTHINGFACTOR2 * (this.CLOSE.getValue(index) - this.EMA2.getValue(index - 1))
            );
          ...
          if (
            this.EMA1.getValue(index) !== null ||
            this.EMA2.getValue(index) !== null ||
            this.MACD.getValue(index) !== null ||
            this.SIGNAL.getValue(index - 1) !== null
          ) {
            this.SIGNAL.setValue(
              index,
              this.SIGNAL.getValue(index - 1) +
                this.SIGNALLINESMOOTHINGFACTOR *
                  (this.MACD.getValue(index) - this.SIGNAL.getValue(index - 1))
            );
            if (this.SIGNAL.getValue(index) === null || this.SIGNAL.getValue(index - 1) === null)
              return;
            var macdPredictor =
              this.SIGNAL.getValue(index) * mp1 -
              this.EMA1.getValue(index) * mp2 +
              this.EMA2.getValue(index) * mp3;
