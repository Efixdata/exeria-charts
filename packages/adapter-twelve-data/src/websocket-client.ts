...
import type { TwelveDataAdapterConfig, TwelveDataPriceEvent } from "./types";
...
export class TwelveDataWebSocketClient {
  ...
  constructor(config: TwelveDataAdapterConfig) {
    if (!config.apiKey) {
      throw new Error("TwelveDataWebSocketClient requires apiKey");
    }

    this.apiKey = config.apiKey;
    const baseWsUrl = config.wsUrl ?? "wss://ws.twelvedata.com/v1/quotes/price";
    const separator = baseWsUrl.includes("?") ? "&" : "?";
    this.wsUrl = `${baseWsUrl}${separator}apikey=${encodeURIComponent(this.apiKey)}`;
  }
        ...
        try {
          const raw =
            typeof event.data === "string"
              ? event.data
              : event.data.toString();
          const data = JSON.parse(raw) as TwelveDataPriceEvent | ControlMessage;
  ...
  private handleMessage(data: TwelveDataPriceEvent | ControlMessage): void {
